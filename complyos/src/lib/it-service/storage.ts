...
export function recomputeState(
  ...
  const applicable = runRuleEngine(profile);
  const applicableIds = getApplicableComplianceIds(applicable);
  let calendar = generateCalendar(profile, applicableIds);
  ...
  const risks = computeRisks(profile.companyId, calendar, evidence);
...
export function loadState(): ITServiceState | null {
  ...
  try {
    ...
    return recomputeState(parsed.profile, parsed.evidence || []);
  } catch {
    return null;
  }
}
...
export function saveProfile(profile: CompanyProfile): ITServiceState {
  ...
  const state = recomputeState(profile, evidence);
  saveState(state);
  return state;
}
...
export function addEvidence(
  ...
  const state = recomputeState(current.profile, [...withoutDuplicate, evidence]);
  saveState(state);
  return state;
}
...
export function markEvidenceComplete(record: {
  ...
  const state = recomputeState(current.profile, [...withoutFocal, evidence]);
  saveState(state);
  return state;
}
