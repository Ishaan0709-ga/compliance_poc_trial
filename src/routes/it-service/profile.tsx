...
import {
  ...
  INDUSTRY_OPTIONS,
  normalizeEntityType,
} from "@/lib/it-service/profile-options";
...
function ProfilePage() {
  ...
  useEffect(() => {
    if (state.profile) {
      setEntityType(normalizeEntityType(state.profile.entityType));
      setIndustry(
        state.profile.industry === "IT Service"
    ...
    const stored = sessionStorage.getItem("its-entity-type");
    if (stored) setEntityType(normalizeEntityType(stored));
  ...
  const selectClass =
    "w-full rounded-lg border border-white/15 bg-white/10 px-3 py-2 text-[14px] outline-none";
      ...
      <div className="mx-auto w-full max-w-2xl flex-1 px-6 pb-20 pt-4">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5 rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
          ...
          {field(
            ...
            "Used by the rule engine to determine applicable compliances."
          )}
          {field(
            "Industry type",
            <select
          ...
          {field(
            ...
            />,
            "India-only release. Multi-country support coming later."
          )}
          {field(
            "Revenue band",
            <select
