...
import { ENTITY_TYPE_OPTIONS } from "@/lib/it-service/profile-options";
...
function OnboardingPage() {
  ...
  const otherEntities = ENTITY_TYPE_OPTIONS.filter(
    (o) => o.value !== "private_limited"
  );
        ...
        <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur">
          {step === "pvt" ? (
            ...
            <>
              ...
              <p className="mt-2 text-[14px] text-white/60">
                Choose your legal entity structure. You can change this later on the company profile.
              </p>
              <div className="mt-8 grid max-h-[320px] gap-2 overflow-y-auto">
                {otherEntities.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
